...
# Plot Graph
# =========================
def plot_rosenbrock_contour_2d(xmin=-2, xmax=2, ymin=-2, ymax=2, levels=50, fname="contour.png"):
    xs = np.linspace(xmin, xmax, 400)
    ys = np.linspace(ymin, ymax, 400)
    X, Y = np.meshgrid(xs, ys)
    Z = 100.0 * (X**2-Y)**2 + (1.0 - X)**2
    plt.figure(figsize=(7, 6))
    cs = plt.contour(X, Y, Z, levels=levels)
    plt.clabel(cs, inline=True, fontsize=8)
    plt.title("Sphere f1 (D=2) Contour")
    plt.xlabel("x1"); plt.ylabel("x2")
    plt.scatter([1],[1], marker="*", s=150 , c='gold', edgecolors='black', zorder=5, label="global min (1,1)")
    plt.legend(); plt.tight_layout()
    plt.savefig(fname, dpi=200); plt.close()

def save_agent_slides_on_contour(pop_history_2d, xmin=-2, xmax=2, ymin=-2, ymax=2, outdir="slides_f2"):
    ...
    for it, pop in enumerate(pop_history_2d, start=1):
        ...
        plt.tight_layout()
        fname = os.path.join(outdir, f"f2_agents_iter_{it:02d}.png")
        plt.savefig(fname, dpi=200); plt.close()
...
def plot_search_history(history, x_range=(-2, 2), y_range=(-2, 2), fname="search_history.png"):
...
def demo_plots_for_D2():
    ...
    plot_rosenbrock_contour_2d(xmin=lb, xmax=ub, ymin=lb, ymax=ub)
    ...
    plot_search_history(logs["pop_history_2d"], x_range=(lb, ub), y_range=(lb, ub), fname="F2_search_history.png")

    plot_first_agent_trajectory(logs["first_agent_traj_x1"], fname="F2_first_agent_trajectory.png")

    plot_average_fitness(logs["avg_fitness"],fname="F2_average_fitness.png")

    plot_full_convergence_curve(logs["convergence"], "F2_convergence_curve.png")
# =========================
...
if __name__ == "__main__":
    ...
    mean_curve, std_curve = run_multiple_trials(
        rosenbrock_f2, lb, ub, D, num_agents, max_iter, num_trials
    )
