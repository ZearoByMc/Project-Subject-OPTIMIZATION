...
if __name__ == "__main__":
    ...
    objective = f1_sphere          # <-- ใช้ objective แท้ ไม่ใช่ *_plots_for_D2
    D = 30
    lb, ub = -5.0, 5.0
    num_agents = 20
    max_iter = 300
    ...
    # ========== F2: Rosenbrock (2D เท่านั้นตามที่นิยาม) ==========
    print("Running Benchmark: F2 - Rosenbrock Function")
    objective = f2_rosenbrock      # <-- 2 มิติเท่านั้น
    D = 2
    lb, ub = -2.0, 2.0
    num_agents = 20
    max_iter = 300
    ...
    D = 20
    lb, ub = -5.0, 5.0   # (มาตรฐานมักใช้ [-5.12, 5.12] ก็ได้)
    num_agents = 20
    max_iter = 300
    num_trials = 10
    mean_curve, std_curve = run_multiple_trials(
    ...
    # เดโม D=2 ของแต่ละฟังก์ชัน (พิมพ์ best min ทดสอบ)
    f1_plots_for_D2()
    f2_plots_for_D2()
    f3_plots_for_D2()
    f4_plots_for_D2()
