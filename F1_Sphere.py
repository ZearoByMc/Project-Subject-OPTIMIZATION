# -*- coding: utf-8 -*-
"""
SCA on Sphere (f1) + Average/Std graph
--------------------------------------
...
# Benchmark: Sphere f1
# =========================
def sphere_f1(x: np.ndarray) -> float:
    """
...
# Plot Graph
# =========================
def plot_sphere_contour_2d(xmin=-5, xmax=5, ymin=-5, ymax=5, levels=50, fname="f1_contour.png"):
    ...
    plt.title("Sphere f1 (D=2) Contour")
...
def save_agent_slides_on_contour(pop_history_2d, xmin=-5, xmax=5, ymin=-5, ymax=5, outdir="slides_f1"):
    ...
    for it, pop in enumerate(pop_history_2d, start=1):
        ...
        plt.tight_layout()
        fname = os.path.join(outdir, f"f1_agents_iter_{it:02d}.png")
        plt.savefig(fname, dpi=200); plt.close()


def plot_convergence(curve, start_iter=5, end_iter=100, fname="f1_convergence.png"):
    n = len(curve)
...
def plot_search_history(history, x_range=(-5, 5), y_range=(-5, 5), fname="F4_search_history.png"):
...
def plot_first_agent_trajectory(trajectory, fname="first_agent_trajectory.png"):
    ...
    plt.plot(trajectory[:100],color='red')
...
def plot_full_convergence_curve(curve, fname="convergence_curve.png"):
...
def demo_plots_for_D2():
    ...
    plot_sphere_contour_2d(xmin=lb, xmax=ub, ymin=lb, ymax=ub)
    _, best, logs = sca(sphere_f1, lb, ub, D, num_agents, max_iter, seed=seed)
    ...
    plot_search_history(logs["pop_history_2d"], x_range=(lb, ub), y_range=(lb, ub), fname="F1_search_history.png")

    plot_first_agent_trajectory(logs["first_agent_traj_x1"], fname="F1_first_agent_trajectory.png")

    plot_average_fitness(logs["avg_fitness"],fname="F1_average_fitness.png")

    plot_full_convergence_curve(logs["convergence"], "F1_convergence_curve.png")
...
if __name__ == "__main__":
    ...
    D = 30
    lb, ub = -5.0, 5.0  # Sphere f1 domain
    num_agents = 20
    max_iter = 300
    num_trials = 10

    mean_curve, std_curve = run_multiple_trials(
        sphere_f1, lb, ub, D, num_agents, max_iter, num_trials
    )
